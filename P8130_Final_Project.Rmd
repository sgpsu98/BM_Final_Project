...
hc_df = 
  hc_df %>% 
  mutate(
    unemployment_num = case_when(
      unemployment == "low" ~ 1,
      unemployment == "high" ~ 2)
  ) %>% 
  mutate(
    urbanization_num = case_when(
      urbanization == "low" ~ 1,
      urbanization == "high" ~ 2)
  ) %>% 
  select(-unemployment, -urbanization) %>% 
  select(-state)
```
...
```{r}
unemployment = 
 lm(rate ~ unemployment_num, data = hc_df)

summary(unemployment)
```
```{r}
unemployment_ = 
 lm(lg ~ unemployment_num, data = hc_df_log)
...
```{r}
urbanization = 
 lm(rate ~ urbanization_num, data = hc_df)
...
```{r}
urbanization_ = 
 lm(lg ~ urbanization_num, data = hc_df_log)
...
# step 1
fit1 <- update(income_reg, . ~ . +unemployment_num)
tidy(fit1)
fit2 <- update(income_reg, . ~ . +urbanization_num)
tidy(fit2)
...
#step 2
fit1 <- update(forward1, . ~ . +unemployment_num)
tidy(fit1)
fit2 <- update(forward1, . ~ . +urbanization_num)
tidy(fit2)
...
#step 3
fit1 <- update(forward2, . ~ . +unemployment_num)
tidy(fit1)
fit2 <- update(forward2, . ~ . +urbanization_num)
tidy(fit2)
...
outcome: rate ~ med_income + high_degree + gini_index
...
##Urbanization -- high_degree
hc_df %>% 
  ggplot(aes(x = high_degree, y = rate, color = factor(urbanization_num))) +
  geom_point() +
...
##Urbanization -- gini
hc_df %>% 
  ggplot(aes(x = gini_index, y = rate, color = factor(urbanization_num))) +
  geom_point() +
  stat_smooth(method = "lm", se = F)
...
```{r}
hc_df %>% 
  ggplot(aes(x = med_income, y = rate, color = factor(urbanization_num))) +
  geom_point() +
  stat_smooth(method = "lm", se = F)
```
...
##Unemployment -- high_degree
hc_df %>% 
  ggplot(aes(x = high_degree, y = rate, color = factor(unemployment_num))) +
  geom_point() +
  stat_smooth(method = "lm", se = F)
...
##Unemployment -- gini
hc_df %>% 
  ggplot(aes(x = gini_index, y = rate, color = factor(unemployment_num))) +
  geom_point() +
  stat_smooth(method = "lm", se = F)
...
```{r}
hc_df %>% 
  ggplot(aes(x = med_income, y = rate, color = factor(unemployment_num))) +
  geom_point() +
  stat_smooth(method = "lm", se = F)
```

# add interaction to aic model

```{r}
inter_model_aic = 
  hc_df %>% 
  lm(rate ~ high_degree + gini_index + high_degree * urbanization_num + high_degree * unemployment_num + gini_index * urbanization_num + gini_index * unemployment_num, data = .)
...
inter_model_aic = 
  hc_df %>% 
  lm(rate ~ high_degree * urbanization_num , data = .)
